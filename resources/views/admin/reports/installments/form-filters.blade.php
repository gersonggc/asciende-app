<div class="row">
    <form method="GET" action={{ route('admin.reports.installments') }} class="col-12">
        @csrf
        ...
        <div class="card-body">
            ...
            <div class="col-12 col-md-6 mb-3 mb-md-0">
                <label for="status" class="form-label mt-0">Estado</label>
                <div class="input-group">
                <div class="input-group-prepend"><span class="input-group-text h-100"><i class="las la-angle-double-down"></i></span></div>
                <select class="form-control" id="status" name="filters[status]">
                    <option value="">Seleccione</option>
                    @foreach($statuses as $key => $status )
                    <option value="{{ $key }}" {{ request('filters.status') == $key ? 'selected' : '' }}>{{ $status }}</option>
                    @endforeach  
                </select>
                </div>
            </div>
            ...
            <div class="row">
                ...
                <a class="btn btn-secondary mr-2" href="{{ route('admin.reports.installments')}}"><i class="fa fa-search"></i> Reiniciar</a>
